...
if (!app) {
  throw new Error("Liquid glass container not found.");
}
...
function drawBackground() {
  ...
  base.addColorStop(0, "#0b0b0b");
  base.addColorStop(0.4, "#141528");
  base.addColorStop(0.7, "#0f1b2e");
  ...
  const waveColors = [
    "rgba(124, 123, 255, 0.5)",
    "rgba(79, 227, 255, 0.4)",
    "rgba(179, 107, 255, 0.35)",
  ];
  for (let i = 0; i < 5; i++) {
    const cx = w * (0.15 + i * 0.18);
    const cy = h * (0.25 + Math.sin(i * 1.2) * 0.2);
    const rg = bgCtx.createRadialGradient(cx, cy, 0, cx, cy, w * 0.42);
    rg.addColorStop(0, waveColors[i % waveColors.length]);
  ...
  bgCtx.fillText("Jonathan P", w * 0.5, h * 0.42);

  const subSize = Math.round(w * 0.02);
  bgCtx.font = `500 ${subSize}px "Space Grotesk", sans-serif`;
  bgCtx.globalAlpha = 0.3;
  bgCtx.fillText("Full Stack Developer", w * 0.5, h * 0.42 + titleSize * 0.7);
  bgCtx.globalAlpha = 1;
  ...
  const scatterSize = Math.round(w * 0.017);
  bgCtx.font = `500 ${scatterSize}px "Space Grotesk", sans-serif`;
  for (let i = 0; i < words.length; i++) {
    bgCtx.fillText(
      words[i],
      w * (0.12 + (i % 4) * 0.25),
      h * (0.12 + Math.floor(i / 4) * 0.35 + (i % 3) * 0.12)
    );
  }
  bgCtx.globalAlpha = 1;
